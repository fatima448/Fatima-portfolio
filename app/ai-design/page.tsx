...
export default function AIDesignPage(){return <main className="editorial-page">
  ...
  <section className="work" id="work"><div className="heading"><h2>Things I&apos;ve made.</h2><small>{projects.length} projects</small></div><div className="projects">{projects.map(project=><a key={project.name} href={project.url} target="_blank" rel="noreferrer"><Image src={`${basePath}${project.image}`} alt={`${project.name} preview`} width={1200} height={675} sizes="(max-width: 760px) 100vw, 58vw"/><div><small>{project.type}</small><h3>{project.name}</h3><ArrowUpRight size={19}/></div><p>{project.desc}</p><footer>{project.tech.map(tech=><span key={tech}>{tech}</span>)}</footer></a>)}</div></section>
